...
def test_batch_processor_init():
    ...
    assert all(ext in processor.extractors for ext in [".pdf", ".docx", ".pptx", ".html"])

@patch("examples.batch_processing.create_extractor")
def test_process_file_success(mock_create_extractor, temp_output_dir, setup_test_files):
    ...
    filename, success, message = processor.process_file(input_file, temp_output_dir)
...
@patch("examples.batch_processing.create_extractor")
def test_process_file_error(mock_create_extractor, temp_output_dir, setup_test_files):
...
@patch("examples.batch_processing.create_extractor")
def test_process_directory(mock_create_extractor, temp_output_dir, setup_test_files):
...
def test_process_directory_empty(temp_output_dir):
    ...
    processor = BatchProcessor()
    successful, failed, errors = processor.process_directory(empty_dir, temp_output_dir)
...
@patch("examples.batch_processing.create_extractor")
def test_process_directory_filtered(mock_create_extractor, temp_output_dir, setup_test_files):
...
@patch("examples.batch_processing.create_extractor")
def test_parallel_processing(mock_create_extractor, temp_output_dir, setup_test_files):
