...
def verify_basic_content(content: str):
    ...
    assert "Description:" in content, "Content should include image descriptions"
...
def verify_pdf_content(content: str):
    ...
    assert "Page 1" in content, "PDF content should include page numbers"
...
def verify_pptx_content(content: str):
    ...
    assert "Slide 1" in content, "PPTX content should include slide numbers"
    if "[Image" in content:
        assert any(
...
def verify_html_content(content: str):
    ...
    assert "Page 1" in content, "HTML content should include page numbers"
    assert "Exploring Nature" in content, "Expected document title not found in HTML"
    assert "interactive" in content.lower(), "Expected interactive elements description not found in HTML"
    assert "biodiversity" in content.lower(), "Expected biodiversity section not found in HTML"
    assert "[Image" in content and "forest" in content.lower(), "Expected forest description not found in HTML"
