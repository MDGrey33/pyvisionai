...
def test_specialized_prompts():
    ...
    assert all(
        prompt in SPECIALIZED_PROMPTS for prompt in expected_prompts
    )
    assert all(
        isinstance(SPECIALIZED_PROMPTS[p], str)
        for p in SPECIALIZED_PROMPTS
    )
...
def test_chart_analysis(
    mock_describe_image, temp_output_dir, setup_test_files
):
    """Test chart analysis."""
    # Setup mock
...
def test_custom_prompt_builder():
    ...
    with patch(
        "examples.custom_prompts.create_extractor"
    ) as mock_create:
        with patch("examples.custom_prompts.print"):
            example_custom_prompt_builder()
...
def test_prompt_combinations():
    ...
    full_prompt = build_prompt(
        has_charts=True, has_tables=True, has_code=True
    )
