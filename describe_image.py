...
def describe_image(image_path: str = DEFAULT_IMAGE_PATH, model: str = "gpt-4o", api_key: Optional[str] = None) -> str:
    """
    ...
    if not api_key:
        raise ValueError("API key is required. Provide it as an argument or set it in the OPENAI_API_KEY environment variable.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate a description for an image using OpenAI API.")
    parser.add_argument("--image_path", type=str, default=DEFAULT_IMAGE_PATH, help="Path to the image file.")
    ...
    try:
        description = describe_image(args.image_path, model=args.model, api_key=args.api_key)
