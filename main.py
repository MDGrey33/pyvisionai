...
def get_user_input():
    source_folder = input(f"Enter the source folder path (default: {DEFAULT_SOURCE_FOLDER}): ")
    if not source_folder:
        source_folder = DEFAULT_SOURCE_FOLDER
    
    output_folder = input(f"Enter the output folder path (default: {DEFAULT_OUTPUT_FOLDER}): ")
    if not output_folder:
        output_folder = DEFAULT_OUTPUT_FOLDER
    
    while True:
        file_type = input(f"Enter file type to process ({'/'.join(SUPPORTED_FILE_TYPES)}): ").lower()
        if file_type in SUPPORTED_FILE_TYPES:
            break
        print(f"Invalid file type. Please enter one of: {', '.join(SUPPORTED_FILE_TYPES)}")
